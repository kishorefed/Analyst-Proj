    ...
    <div className="flex flex-1 flex-col">
      ...
      <div className="grid grid-cols-2 gap-3.5">
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`${inputBase} ${errors.client ? inputError : 'border-[#dceaea]'}`}
            />
          </div>
          {errors.client && (
            <div className="text-[11px] text-red-600">⚠ Client name is required</div>
          )}
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`${inputBase} ${errors.project ? inputError : 'border-[#dceaea]'}`}
            />
          </div>
          {errors.project && (
            <div className="text-[11px] text-red-600">⚠ Project name is required</div>
          )}
        </div>
          ...
          <div className="grid grid-cols-4 gap-2.5">
            {DOMAINS.map((d) => (
              ...
              >
                ...
                <div className="mt-1 text-[11px] font-bold leading-tight">{d.value}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="flex flex-col gap-1.5">
          ...
          {errors.type && (
            <div className="text-[11px] text-red-600">⚠ Please select a project type</div>
          )}
        </div>
      ...
      <div className="mt-auto flex items-center justify-between border-t border-[#eef4f4] pt-6">
        <button
          type="button"
          onClick={onBack}
          className="flex items-center gap-2 rounded-lg border-2 border-[#dceaea] bg-white px-5 py-2.5 font-['DM_Sans',sans-serif] text-[13px] font-semibold text-[#4d6b6b] transition-colors hover:border-teal-400 hover:bg-teal-50 hover:text-teal-700"
        >
          ← Back
        </button>
        <button
          type="button"
          onClick={onNext}
          className="flex items-center gap-2 rounded-xl bg-linear-to-br from-teal-700 to-teal-400 px-6 py-3 font-['Syne',sans-serif] text-sm font-bold text-white shadow-[0_6px_22px_rgba(15,118,110,.3)] transition-all hover:-translate-y-0.5 hover:shadow-[0_10px_30px_rgba(15,118,110,.42)]"
        >
          Next: Stakeholders <span>→</span>
        </button>
      </div>
    </div>
  );
